...
describe("Pokemon Service", () => {
  ...
  const pokemonResponse = {
    count: 1118,
    next: "https://pokeapi.co/api/v2/pokemon/?offset=807&limit=311",
    previous: null,
    results: [
      {
        name: "bulbasaur",
  ...
  beforeEach(() => {
    component = new DataService(httpMock as any);
  });
  describe("Setup Component", () => {
    describe("Fetch Pokemon", () => {
      // Done will tell the code when it's over
      it("should return the data of pokemon", (done) => {
        httpMock.get.mockImplementationOnce(() => of(pokemonResponse));
        component
          .getPokemons()
          .pipe(
            tap((pokemon) => {
              expect(pokemon).toEqual(pokemonResponse);
              done();
            })
          )
          .subscribe();
    ...
    describe("fetch users", () => {
      it("the name of the user should be leanne Graham", (done) => {
        httpMock.get.mockImplementationOnce(() => of(userResponse));
        component
          .getUser()
          .pipe(
            tap((user) => {
      ...
      // Axios
      // it("axios should work ", async () => {
      //   expect.assertions(1);
      //   const data = await component.fetchUser();
